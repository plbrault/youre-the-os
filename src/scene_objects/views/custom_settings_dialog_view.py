...
class CustomSettingsDialogView(ModalView):
    ...
    def x(self, value):
        ...
        dialog._num_cpus_selector.view.x = right_edge - dialog._num_cpus_selector.view.width
        dialog._num_processes_at_startup_selector.view.x = (
            right_edge - dialog._num_processes_at_startup_selector.view.width)
        dialog._max_processes_selector.view.x = (
            right_edge - dialog._max_processes_selector.view.width)
        dialog._num_ram_rows_selector.view.x = (
            right_edge - dialog._num_ram_rows_selector.view.width)
        dialog._swap_delay_selector.view.x = (
            right_edge - dialog._swap_delay_selector.view.width)
        dialog._parallel_swap_selector.view.x = (
            right_edge - dialog._parallel_swap_selector.view.width)
        dialog._new_process_probability_selector.view.x = (
            right_edge - dialog._new_process_probability_selector.view.width)
        dialog._priority_process_probability_selector.view.x = (
            right_edge - dialog._priority_process_probability_selector.view.width)
        dialog._io_probability_selector.view.x = (
            right_edge - dialog._io_probability_selector.view.width)
        dialog._graceful_termination_selector.view.x = (
            right_edge - dialog._graceful_termination_selector.view.width)
        dialog._start_button.view.x = (
            self.x + (self.width / 2) - dialog._start_button.view.width - 10)
        dialog._cancel_button.view.x = self.x + (self.width / 2) + 10
    ...
    def y(self, value):
        ...
        dialog._num_cpus_selector.view.y = (
            self.num_cpus_y + (label_h - dialog._num_cpus_selector.view.height) / 2)
        dialog._num_processes_at_startup_selector.view.y = (
            self.num_processes_at_startup_y
            + (label_h - dialog._num_processes_at_startup_selector.view.height) / 2)
        dialog._max_processes_selector.view.y = (
            self.max_processes_y + (label_h - dialog._max_processes_selector.view.height) / 2)
        dialog._num_ram_rows_selector.view.y = (
            self.num_ram_rows_y + (label_h - dialog._num_ram_rows_selector.view.height) / 2)
        dialog._swap_delay_selector.view.y = (
            self.swap_delay_y + (label_h - dialog._swap_delay_selector.view.height) / 2)
        dialog._parallel_swap_selector.view.y = (
            self.parallel_swap_y + (label_h - dialog._parallel_swap_selector.view.height) / 2)
        dialog._new_process_probability_selector.view.y = (
            self.new_process_probability_y
            + (label_h - dialog._new_process_probability_selector.view.height) / 2)
        dialog._priority_process_probability_selector.view.y = (
            self.priority_process_probability_y
            + (label_h - dialog._priority_process_probability_selector.view.height) / 2)
        dialog._io_probability_selector.view.y = (
            self.io_probability_y + (label_h - dialog._io_probability_selector.view.height) / 2)
        dialog._graceful_termination_selector.view.y = (
            self.graceful_termination_y
            + (label_h - dialog._graceful_termination_selector.view.height) / 2)
        dialog._start_button.view.y = self.y + self.height - dialog._start_button.view.height - 18
        dialog._cancel_button.view.y = self.y + self.height - dialog._cancel_button.view.height - 18
