...
def event_io_queue(count):
    add_event(EventType.IO_QUEUE, {
        'io_count': count
    })

def event_page_swap(pid, idx, swap):
    add_event(EventType.PAGE_SWAP, {
        'pid': pid,
        'idx': idx,
        'swap': swap
    })
